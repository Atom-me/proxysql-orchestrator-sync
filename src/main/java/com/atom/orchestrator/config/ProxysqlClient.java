...
public class ProxysqlClient {
    private final static Logger LOGGER = LoggerFactory.getLogger(ProxysqlClient.class);
    ...
    public static boolean execute(String ip, Integer port, String database, String user, String pass, List<String> sqlList, Integer timeout, boolean transaction) {
        ...
        if (StringUtils.isBlank(ip) || port == null || StringUtils.isBlank(user) || StringUtils.isBlank(pass)) {
            throw new RuntimeException("参数错误：ip/port/user/pass不能为空");
        }
        ...
        try {
            ...
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, StringUtils.join(sqlList, ";")), e);
            Thread.currentThread().interrupt();
            throw new RuntimeException(String.format(EXECUTE_SQL_ERROR, StringUtils.join(sqlList, ";")) + ": InterruptedException");
        } catch (ExecutionException e) {
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, StringUtils.join(sqlList, ";")), e);
            throw new RuntimeException(String.format(EXECUTE_SQL_ERROR, StringUtils.join(sqlList, ";")) + ": " + e.getMessage());
        } catch (TimeoutException e) {
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, StringUtils.join(sqlList, ";")), e);
            throw new RuntimeException(String.format(EXECUTE_SQL_TIMEOUT, StringUtils.join(sqlList, ";")) + "：" + timeout + "s");
        }
    ...
    public static List<Map<String, Object>> executeQuery(String ip, Integer port, String database, String user, String pass, String sqlString, Integer timeout) {
        ...
        try {
            ...
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, sqlString), e);
            Thread.currentThread().interrupt();
            throw new RuntimeException(String.format(EXECUTE_SQL_ERROR, sqlString) + ": InterruptedException");
        } catch (ExecutionException e) {
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, sqlString), e);
            throw new RuntimeException(String.format(EXECUTE_SQL_ERROR, sqlString) + ": " + e.getMessage());
        } catch (TimeoutException e) {
            LOGGER.error(String.format(EXECUTE_SQL_ERROR, sqlString), e);
            throw new RuntimeException(String.format(EXECUTE_SQL_TIMEOUT, sqlString) + "：" + timeout + "s");
        }
...
//        List<Map<String, Object>> maps = executeQuery("8.140.50.73", 6032, "main", "atom", "atompassword",
//                "select * from mysql_collations",
//                null);
//        LOGGER.info(JSON.toJSONString(maps));
//        //ddl & batch execute
////        LOGGER.info(JSON.toJSONString(execute("8.140.50.73", 6032, "main", "atom", "atompassword",
